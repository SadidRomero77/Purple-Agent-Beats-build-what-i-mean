...
YOUR TASK:
Given a building instruction and the current grid state, decompose the instruction into a sequence of atomic build steps. Output valid JSON only.
...
- Use full 3D (x,y,z) coordinates in your spatial reasoning to verify logic, but output only {"x":..., "z":...} in the JSON steps.

RULES:
- COLOR HANDLING (important):
  * If a step's color is EXPLICITLY stated in the instruction -> use that color.
  * If blocks are mentioned WITHOUT a color but the instruction mentions other colors -> use "Uncolored". Do NOT guess or infer. We will ask the user for the right color.
  * E.g., "Put a red block in each corner, then stack two blocks on top" -> the corner blocks are "Red", the top blocks are "Uncolored" (not stated, could be any color).
  * E.g., "Build three blue blocks, then stack blocks to the left" -> the blue blocks are "Blue", the left stack is "Uncolored".
...
- "extend_row" means place N blocks in a line starting FROM a position (inclusive).
  ...
  Example: "row of 3 starting at origin going left" -> start at (0,0), places at (0,0), (-100,0), (-200,0).
...
IMPORTANT PATTERNS:
- "Along the left edge" = 9 blocks at x=-400 with z from -400 to 400. Use extend_row with count=9, position x=-400 z=-400, direction=front.
- "Along the right edge" = 9 blocks at x=400, same z range.
- "Along the top/back edge" = 9 blocks at z=-400 with x from -400 to 400. Use extend_row with count=9, position x=-400 z=-400, direction=right.
...
- When an instruction says "build a stack of N blocks to the left of existing," the NEW stack should be at (existing_x - 100, existing_z). It's ONE step away.
...
- "highlighted square" / "middle square" / "highlighted center square" = the origin = (0, 0).
  "Starting from the highlighted square" means the FIRST block goes AT (0, 0).
  "Starting from the middle square and going to the right" = extend_row starting AT x=0, z=0, direction=right. NOT at x=100.
- "to the right of the origin" = x=100. "Starting at the square to the right of the origin" means first block at (100, 0).
- "each end" / "both ends" of a row = the leftmost AND rightmost blocks of that row.
  After extending a row, recalculate the ends! E.g., if start row is at x=[-100,0,100] and we add 2 to the right, extended row = [-100,0,100,200,300]. Ends are x=-100 and x=300.
- "in front of the leftmost block" = find the block with lowest x, then go +z from it.
  E.g., leftmost block at (-100, 0) -> "in front of" = (-100, 100).
- DIRECTION DOUBLE-CHECK: Before outputting, verify "going to the left" uses direction "left" (decreasing x), "going to the right" uses direction "right" (increasing x).
...
- Example: "red on left of green at (0,0), yellow on left of red" -> red at (-100,0), yellow at (-200,0). Each step shifts x by -100.

CHAIN REFERENCES -- track positions step by step:
- When step A says "stack 3 green blocks behind the existing green block at (0,0)" the green stack is now at (0,-100).
- Step B says "build a yellow stack to the right of the green one" -> the reference "the green one" is the stack at (0,-100), so yellow goes at (100,-100).
- WRONG: placing yellow at (100,0) -- that's right of the ORIGINAL block, not the stack.
- Similarly: "behind the rightmost blue block at (100,0), build red" -> red at (100,-100). "Build yellow directly to the right of the red one" -> yellow at (200,-100).
- "To the right of the ROW" = at position (rightmost_x + 100, z), starting at y=50 (ground level). Do NOT stack on top of a block IN the row. The new stack goes AFTER the row's last block. Row at x=[100,200,300] -> "stack to the right of this row" = x=400, z same. WRONG: placing a stack ON TOP of row block at x=200.
...
- An L at positions [(0,-100),(0,0),(0,100),(100,100)] has longer arm along z-axis (3 blocks: z=-100,0,100 at x=0) and short arm along x-axis (1 block at x=100, z=100). Junction at z=100 where the arms meet. Extend longer away from junction: (0,-200) and (0,-300). NOT (0,200) -- that goes through the junction and breaks the L-shape. "Add block to shorter side" = continue outward -> (200,100).
- NEVER stack on top when told to "extend" -- extending is always horizontal.
- T shape: the "base" / "longer base" is the stem (longer arm). To identify: count blocks along each axis -- whichever axis has MORE is the "longer base" / stem. "Arms" are the endpoints of the crossbar. "Add to each arm" = add at the ends of the crossbar, extending outward, not on top.

IDENTIFYING EXISTING STRUCTURES (do this BEFORE placing relative to anything):
When the instruction says "relative to", "next to", "behind the row", "to the left of the stack", etc., your FIRST reasoning step must IDENTIFY the reference structure from the grid state:

For a single block or stack:
  1. Find the block(s) matching the description (color, position words like "leftmost").
  2. Note its coordinates: (x, z). For a stack, note the base (x, z) and height.
...
For a row:
  ...
  2. Determine which axis varies -> that is the row's axis and direction.
     E.g., blocks at (-100,0), (0,0), (100,0) -> z is fixed at 0, x varies -> this is an x-row.
...
Example identification reasoning:
  Grid has: Red at (-200,100), (-100,100), (0,100), (100,100).
...
Grid: Red block at (200,-100).
Reasoning:
1. "in front of" = +z. Existing red at (200,-100) -> new stack at x=200, z=-100+100=0. Two blocks auto-stack at y=50,150.
...
Grid: empty.
Reasoning:
1. Highlighted square = origin -> green at x=0, z=0. "to its left" -> green at x=-100, z=0.
2. "behind each green" = -z for each -> yellow at x=0, z=-100 and x=-100, z=-100.
Output:
{"steps":[
...
Reasoning:
1. "back edge" = fixed z=-400, x varies from -200 to 200 (5 positions).
...
Grid: Red T-shape with blocks at (100,0),(200,0),(300,0),(200,-100),(200,-200).
Reasoning:
1. FIND THE JUNCTION: look for the block shared by two perpendicular lines.
   ...
   They share (200,0) -> JUNCTION = (200,0).
...
3. FIND BASE AND DIRECTION:
   Base = stem end farthest from junction = (200,-200).
...
4. EXTEND: add 2 blocks past base in direction "behind".
   Past z=-200, continue -z: next positions (200,-300) and (200,-400).
5. ARMS = crossbar tips. Left tip (100,0), right tip (300,0).
   Extend outward along crossbar axis (x-axis): (0,0) and (400,0).
Output:
{"steps":[
...
Grid: Yellow blocks at (0,-200) and (100,-200).
Reasoning:
1. Yellow row at z=-200, x goes from 0 to 100. "going right" = +x direction. Extend right from x=200 and x=300.
2. Extended row spans x=0 to x=300. Ends: x=0 and x=300.
3. "block on each end" = stack 1 block on each end. Color not stated -> "Uncolored".
Output:
{"steps":[
...
Grid: empty.
Reasoning:
1. "to the right of the center square" -> center=(0,0), right=+x -> x=100, z=0. Stack 2 yellow.
2. "to the right of it" -> x=200, z=0. "same height" = 2.
Output:
{"steps":[
...
Grid: empty.
Reasoning:
1. "going left" = direction left (-x). Start at origin x=0, z=0: row at x=0, -100, -200, -300, -400.
2. "last green block you placed" = last in row = x=-400, z=0.
3. Stack 4 on top at x=-400, z=0. Color not stated -> "Uncolored".
Output:
{"steps":[
...
Grid: Blue blocks at (-100,0), (0,0), (100,0).
Reasoning:
1. IDENTIFY the row: 3 blue blocks share z=0, x varies -100->0->100 -> x-row at z=0. Leftmost: x=-100. Rightmost: x=100.
2. "extend by two" -- row runs in +x, continue: x=200 and x=300. Two new blue blocks at z=0.
3. "leftmost blue block" = x=-100, z=0 (unchanged by the rightward extension).
4. "behind the leftmost" = -z direction -> z=-100, x=-100. Stack 2 green.
Output:
{"steps":[
